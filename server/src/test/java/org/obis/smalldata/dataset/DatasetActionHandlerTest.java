...
public class DatasetActionHandlerTest {
  ...
  void getAllDataSets(Vertx vertx, VertxTestContext testContext) {
    vertx
        ...
        .<JsonArray>send(
            "datasets",
            new JsonObject(Map.of(KEY_ACTION, "find", "query", new JsonObject())),
            ar -> {
              ...
              assertThat(refs)
                  .containsExactlyInAnyOrder(
                      "NnqVLwIyPn-nRkc", "wEaBfmFyQhYCdsk", "ntDOtUc7XsRrIus", "PoJnGNMaxsupE4w");
  ...
  void getOneDataset(Vertx vertx, VertxTestContext testContext) {
    ...
    vertx
        ...
        .<JsonArray>send(
            "datasets",
            new JsonObject(
                Map.of(KEY_ACTION, "find", "query", new JsonObject().put(QUERY_REF, ref))),
            ar -> {
              var datasets = ar.result().body();
  ...
  void getNotAvailableDataset(Vertx vertx, VertxTestContext testContext) {
    vertx
        ...
        .<JsonArray>send(
            "datasets",
            new JsonObject(
                Map.of(KEY_ACTION, "find", "query", new JsonObject().put(QUERY_REF, "unknown"))),
            ar -> {
              var datasets = ar.result().body();
  ...
  void insertDataset(Vertx vertx, VertxTestContext testContext) {
    vertx
        ...
        .<JsonObject>send(
            ...
            new JsonObject(
                Map.of(KEY_ACTION, "insert", "dataset", new JsonObject().put("key", 42))),
            ar -> {
              if (ar.succeeded()) {
        ...
        .<JsonObject>send(
            ...
            new JsonObject(
                Map.of(
                    ...
                    "PoJnGNMaxsupE4w",
                    "dataset",
                    new JsonObject().put("universe_key", 42))),
            ar -> {
              if (ar.succeeded()) {
                assertThat(ar.succeeded()).isTrue();
                var json = ar.result().body();
