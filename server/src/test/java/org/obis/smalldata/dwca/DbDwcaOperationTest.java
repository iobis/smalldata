...
class DbDwcaOperationTest {
  ...
  private static final JsonObject DATASET_QUERY =
      new JsonObject().put(DATASET_REF_KEY, DATASET_REF_DEFAULT_VALUE);
  ...
  void findDwcaRecordsReturnsAllKeysIfFieldsIsEmpty(VertxTestContext testContext) {
    var dwcaRecords = dbDwcaOperation.findDwcaRecords(DATASET_QUERY, new JsonObject());
    dwcaRecords.setHandler(
        ar -> {
          var result = ar.result();
          assertThat(result)
              ...
              .allSatisfy(
                  dwca -> {
                    assertThat(dwca.getMap())
                        .containsOnlyKeys(
                            "_ref", USER_REF_KEY, "_id", DATASET_REF_KEY, DWC_TABLE, DWC_RECORD);
                    assertThat(dwca.getJsonObject(DWC_RECORD).getMap())
                        .containsOnlyKeys("id", "tdwg", "purl", IOBIS_KEY);
                  });
          testContext.completeNow();
        });
  }
  ...
  @Test
  void findDwcaRecordsAlwaysProvidesDefaultSetOfKeysNextToRequiredOnes(
      VertxTestContext testContext) {
    var dwcaRecords =
        dbDwcaOperation.findDwcaRecords(
            DATASET_QUERY, new JsonObject().put("_ref", true).put(USER_REF_KEY, false));
    dwcaRecords.setHandler(
        ar -> {
  ...
  void findDwcaRecordsProvidesNestedData(VertxTestContext testContext) {
    var dwcaRecords =
        dbDwcaOperation.findDwcaRecords(
            DATASET_QUERY, new JsonObject().put("dwcRecord.tdwg", true));
    dwcaRecords.setHandler(
        ar -> {
          assertThat(ar.result())
              .hasSize(642)
              .allSatisfy(
                  dwca -> {
                    assertThat(dwca.getMap())
                        .containsOnlyKeys(
                            USER_REF_KEY, "_id", DATASET_REF_KEY, DWC_TABLE, DWC_RECORD);
                    assertThat(dwca.getJsonObject(DWC_RECORD).getMap())
                        .containsOnlyKeys("id", "tdwg");
                  });
          testContext.completeNow();
        });
  }
  ...
  void findDwcaRecordsForUnknownDatasetRef(VertxTestContext testContext) {
    var dwcaRecords =
        dbDwcaOperation.findDwcaRecords(new JsonObject().put(DATASET_REF_KEY, "unknown"));
    dwcaRecords.setHandler(
        ar -> {
          var result = ar.result();
  ...
  void findDwcaRecordsForUserRef(VertxTestContext testContext) {
    var dwcaRecords =
        dbDwcaOperation.findDwcaRecords(new JsonObject().put(USER_REF_KEY, USER_REF_DEFAULT_VALUE));
    dwcaRecords.setHandler(
        ar -> {
          var result = ar.result();
  ...
  void findDwcaRecordsForUnknownUserRef(VertxTestContext testContext) {
    var dwcaRecords =
        dbDwcaOperation.findDwcaRecords(new JsonObject().put(USER_REF_KEY, "unknown"));
    dwcaRecords.setHandler(
        ar -> {
          var result = ar.result();
          assertThat(result).isEmpty();
    ...
    var records =
        List.of(
            new JsonObject()
                ...
                .put(
                    DWC_RECORD,
                    new JsonObject().put("id", dwcaId).put(IOBIS_KEY, new JsonObject())),
            new JsonObject()
                .put(DWC_TABLE, "emof")
                .put(USER_REF_KEY, USER_REF_DEFAULT_VALUE)
                .put(DATASET_REF_KEY, DATASET_REF_DEFAULT_VALUE)
          ...
          mongoClient.find(
              ...
              arFind -> {
                ...
                foundRecords.forEach(
                    found -> {
                      assertThat(found.getJsonObject(DWC_RECORD).getJsonObject(IOBIS_KEY))
