...
public class EmlGeneratorTest {
  ...
  void generateDataSetEmlMapsFromJsonObject() throws IOException {
    ...
    var json =
        mapper.<List<Map<String, Object>>>readValue(
            datasets, new TypeReference<List<Map<String, Object>>>() {});
    ...
    var dataSetEmlMaps =
        json.stream()
            ...
            .map(
                dataset ->
                    emlGenerator.generate(
                        dataset, "http://localhost:8080/" + dataset.getString("_ref")))
            .map(Optional::get);
  ...
  private static Diff diffWithExpectedXml(DataSetEmlMap dataSetEmlMap) {
    return DiffBuilder.compare(dataSetEmlMap.getEml())
