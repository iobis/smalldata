...
public class UserHandlerTest {
            ...
            ar -> {
              if (ar.succeeded()) {
                ...
                assertThat(user.getJsonArray(KEY_DATASET_REFS))
                    .hasSize(4)
                    .containsExactly(
                        "wEaBfmFyQhYCdsk", "ntDOtUc7XsRrIus", "PoJnGNMaxsupE4w", "NnqVLwIyPn-nRkc");
        ...
        .<JsonArray>send(
            ...
            new JsonObject(
                Map.of(
                    ...
                    "find",
                    "query",
                    new JsonObject().put(KEY_EMAIL_ADDRESS, "some.user@domain.org"))),
            ar -> {
              if (ar.succeeded()) {
                var body = ar.result().body();
        ...
        .<JsonObject>send(
            ...
            new JsonObject(
                Map.of(
                    ...
                    "insert",
                    "user",
                    new JsonObject().put(KEY_EMAIL_ADDRESS, DEFAULT_EMAIL_ADDRESS))),
            ar -> {
              if (ar.succeeded()) {
                ...
                assertThat(body.getJsonObject(KEY_BULKINESS).getDouble(KEY_VALUE)).isEqualTo(0.0);
                testContext.completeNow();
              } else {
                testContext.failNow(ar.cause());
              }
  ...
  void increaseBulkiness(Vertx vertx, VertxTestContext testContext) {
    vertx
        ...
        .<JsonObject>send(
            "users.bulkiness",
            new JsonObject().put("action", "increase").put("userRef", "FsfEMwhUTO_8I68"),
            ar -> {
              if (ar.succeeded()) {
                var bulkiness = ar.result().body().getJsonObject("bulkiness");
                assertThat(bulkiness.getDouble("value")).isBetween(1.0, 23.5711);
                assertThat(bulkiness.getInstant("instant"))
                    .isBetween(Instant.now().minusMillis(200), Instant.now());
                testContext.completeNow();
              } else {
                testContext.failNow(ar.cause());
              }
