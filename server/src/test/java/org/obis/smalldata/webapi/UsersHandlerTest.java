...
public class UsersHandlerTest {
  ...
  void getUsers(Vertx vertx, VertxTestContext context) {
    ...
    vertx
        ...
        .localConsumer(
            ...
            message ->
                message.reply(
                    new JsonArray().add(new JsonObject().put(KEY_USERS_REF, "some-user-ref"))));
    client
  ...
  void postUserWithSupportedRoles(String role, Vertx vertx, VertxTestContext context) {
    ...
    client
        ...
        .sendJson(
            new JsonObject().put(KEY_ROLE, role).put(KEY_EMAIL_ADDRESS, DEFAULT_EMAIL_ADDRESS),
            ar -> {
              try {
                assertThat(ar.succeeded()).isTrue();
                var json = ar.result().body();
            ...
            ar -> {
              try {
                ...
                assertThat(json.getMap())
                    ...
                    .containsEntry(
                        "exceptionMessage",
                        "$.role: does not have a value in the enumeration [node manager, researcher]");
              } catch (AssertionError e) {
                context.failNow(e);
              }
              context.completeNow();
  ...
  private void usersConsumer(Vertx vertx) {
    usersConsumer(
        vertx, new JsonArray().add(new JsonObject().put(KEY_EMAIL_ADDRESS, DEFAULT_EMAIL_ADDRESS)));
  }
