...
public class DwcaRecordsHandlerTest extends DefaultHandlerTest {
    ...
    vertx
        ...
        .localConsumer(
            ...
            message ->
                message.reply(
                    new JsonArray()
                        .add(new JsonObject().put(KEY_DWCA_ID, SecureRandomString.generateId()))));
    httpGetJsonBody(
        client,
        url,
        ar -> {
  ...
  void testPostHandler(Vertx vertx, VertxTestContext context) {
    ...
    vertx
        ...
        .localConsumer(
            "dwca.record",
            message ->
                message.reply(new JsonObject().put(KEY_DWCA_ID, SecureRandomString.generateId())));
    httpPostJsonBody(
        client,
        url,
        new JsonObject(
            Map.of("core", OCCURRENCE, OCCURRENCE, new JsonArray().add(new JsonObject()))),
        ar -> {
          assertThat(ar.succeeded()).isTrue();
          var json = ar.result().body();
          assertThat(json.containsKey(KEY_DWCA_ID)).isTrue();
    ...
    httpPostJsonBody(
        ...
        new JsonObject(
            Map.of(
                ...
                OCCURRENCE,
                OCCURRENCE,
                new JsonArray(List.of(new JsonObject(), new JsonObject())))),
        ar -> {
          assertThat(ar.succeeded()).isTrue();
          assertThat(ar.result().statusCode()).isEqualTo(422);
          assertThat(ar.result().statusMessage()).isEqualTo("Invalid request body");
          assertThat(ar.result().body().getJsonArray("messages"))
              .containsOnly("core table 'occurrence' can have only 1 record.");
          context.completeNow();
        });
  }
  ...
  void testPostHandlerMultipleMessages(Vertx vertx, VertxTestContext context) {
    ...
    httpPostJsonBody(
        client,
        url,
        new JsonObject().put("core", OCCURRENCE).put(OCCURRENCE, new JsonArray()),
        ar -> {
          assertThat(ar.succeeded()).isTrue();
          assertThat(ar.result().statusCode()).isEqualTo(422);
          assertThat(ar.result().statusMessage()).isEqualTo("Invalid request body");
            ...
            message -> {
              switch (message.body().getString("action")) {
                case "find":
                  message.reply(
                      new JsonArray()
                          .add(new JsonObject().put(KEY_EMAIL_ADDRESS, DEFAULT_EMAIL_ADDRESS)));
                  break;
                case "insert":
                case "replace":
