...
public class DefaultHandlerTest {
  ...
  protected static final JsonObject CONFIG =
      new JsonObject()
          .put("mode", "DEMO")
          .put("auth", new JsonObject().put("provider", "demo").put("demokey", "verysecret"))
          .put("http", new JsonObject().put("port", HTTP_PORT));

  protected void httpGetJsonBody(
      WebClient client, String url, Handler<AsyncResult<HttpResponse<JsonObject>>> handler) {
    client
  ...
  protected void httpGetString(
      WebClient client, String url, Handler<AsyncResult<HttpResponse<String>>> handler) {
    client
        .get(HTTP_PORT, LOCALHOST, url)
        .putHeader("Authorization", "Basic verysecret")
        .as(BodyCodec.string())
