...
public class AuthComponent extends AbstractVerticle {
  ...
  public void start(Future<Void> startFuture) {
    ...
    try {
      ...
      error(
          e,
          "Cannot start AuthComponent component: possibly wrong algorithm or keys for JWT signing/verification?");
      startFuture.fail(e);
    }
  }
  ...
  private AuthProvider generateAuthProvider()
    ...
    if ("local".equals(config().getString("provider", "local"))) {
      ...
      if (isNullOrBlank(config().getString(VERIFY_KEY))
          || isNullOrBlank(config().getString(SIGN_KEY))) {
      ...
      if (isNullOrBlank(config().getString(VERIFY_KEY))) {
        throw new InvalidKeyException(
            "Need to provide a public key for non-local JWT authorization");
      } else {
        var pubSecKey =
            new PubSecKeyOptions()
                .setAlgorithm(config().getString(ALG_KEY, AUTH_ES256))
  ...
  private static String createSecretKey(KeyPair keyPair) {
    return new String(
        Base64.getEncoder().encode(keyPair.getPrivate().getEncoded()), StandardCharsets.UTF_8);
  }
