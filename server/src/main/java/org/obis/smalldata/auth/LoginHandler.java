...
class LoginHandler {
  ...
  void login(
      ...
      BiConsumer<Integer, String> failConsumer) {
    var username = "paulo";
    var password = "secret";
    if (username.equals(message.getString("username"))
        && password.equals(message.getString("password"))) {
      var token =
          authProvider.generateToken(
              new JsonObject().put("aud", "occurrences-OBIS").put("sub", "paulo"),
  ...
  void verifyToken(Message<JsonObject> message) {
    verifyToken(message.body(), user -> message.reply(user.principal()), message::fail);
  }

  void verifyToken(
      JsonObject message,
      Consumer<User> successConsumer,
      BiConsumer<Integer, String> failConsumer) {
