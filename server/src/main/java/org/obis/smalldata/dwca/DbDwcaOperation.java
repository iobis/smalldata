...
class DbDwcaOperation {
  ...
  private static final JsonObject DEFAULT_DWCA_FIELDS =
      new JsonObject()
          .put("dwcRecord.id", true)
          .put("addedAtInstant", true)
          .put("user_ref", true)
          .put("dwcTable", true)
  ...
  Future<List<JsonObject>> findDwcaRecords(JsonObject query, JsonObject fields) {
    ...
    var fieldProjection = fields.size() == 0 ? fields : fields.mergeIn(DEFAULT_DWCA_FIELDS);
  ...
  Future<JsonObject> putDwcaRecord(String dwcaId, List<JsonObject> records) {
    ...
    mongoClient.findWithOptions(
        ...
        arFind -> {
          var oldIds =
              arFind.result().stream().map(id -> id.getString("_id")).collect(Collectors.toList());
          var operations =
              oldIds
                  .stream()
          ...
          operations.addAll(
              records.stream().map(BulkOperation::createInsert).collect(Collectors.toList()));
  ...
  Future<Map<String, String>> coreTableMap() {
    ...
    mongoClient.findWithOptions(
        ...
        new JsonObject(),
        new FindOptions()
            .setFields(new JsonObject().put("meta.dwcTables.core", true).put(KEY_REF, true)),
        ar ->
            coreTableMap.complete(
                ar.result()
                    .stream()
                    .map(
                        dataset ->
                            new AbstractMap.SimpleEntry<>(
                                dataset.getString(KEY_REF), extractCoreTable(dataset)))
