...
public class RssComponent extends AbstractVerticle {
  ...
  public void start() {
    ...
    var filterPeriodicityMap = Map.<String, Supplier<Instant>>ofEntries(
        entry("hourly", () -> Instant.now().minus(1, ChronoUnit.HOURS)),
        entry("daily", () -> Instant.now().minus(1, ChronoUnit.DAYS)),
        entry("weekly", () -> LocalDateTime.ofInstant(Instant.now(), ZoneOffset.UTC)
            .minus(1, ChronoUnit.WEEKS).toInstant(ZoneOffset.UTC)),
        entry("monthly", () -> LocalDateTime.ofInstant(Instant.now(), ZoneOffset.UTC)
            .minus(1, ChronoUnit.MONTHS).toInstant(ZoneOffset.UTC)),
        entry("yearly", () -> LocalDateTime.ofInstant(Instant.now(), ZoneOffset.UTC)
            .minus(1, ChronoUnit.YEARS).toInstant(ZoneOffset.UTC)),
    ...
    vertx.eventBus().<JsonObject>consumer("internal.rss", message ->
        dbOperation.withAggregatedDatasets(
            res -> {
              var filterFrom = filterPeriodicityMap.get(message.body().getString("periodicity")).get();
              ...
              try {
                ...
                message.fail(500,
                    "Could nor read datasets" + ex.getMessage());
              }
            })
    );
  }
