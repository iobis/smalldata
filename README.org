...
*** Installing a web client
    ...
    After a web client has been built, installing is just making a copy in the right directory, usually something like:

    ~/var/www/html/admin~ and ~/var/www/html/occurrences~ for the admin and occurrences respectively. The location
    should match the one that's configured in your reverse proxy (e.g. nginx, Apache, ...).
